...
Also ensure the destination bucket you configure (see `BUCKET` below) exists and that you have
...
| `BUCKET`                  | Raw bucket (no `gs://`)                        | *set to your own bucket — see `scripts/backlog_bear.sh`* |
...
5. **Upload to GCS** - Uploads partitions with manifests to `gs://$BUCKET/ecom/raw` (your configured bucket)
...
export BUCKET="your-bucket-name-raw"
...
```bash
gcloud storage du gs://$BUCKET --recursive --summarize --human-readable
```
